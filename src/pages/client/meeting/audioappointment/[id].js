...
import styles from "@/styles/professional/meeting/Audioappointment.module.css";
...
const videoappointment = () => {
  ...
  const onMessage = (messages) => {
    ...
    if (isJSONString(messages?.incomingMessageRequest?.message?.body)) {
      ...
      console.log("ESTABLISHED", receivedData.action === "ESTABLISHED");
  ...
  const CallSubmitCall = async () => {
    if (route.asPath.includes("professional")) {
      const end = new Date();
      const startTimeData = convertToUTCCurrentTimeZone(startTime);
      axiosInstance
        .post(`${API.walletPaymentComplete}/${scedule_id}`, {
          call_start_time: startTimeData,
          call_end_time: convertToUTCCurrentTimeZone(end.toISOString()),
        })
        .then((res) => {
          if (res)
            route.push(
    ...
    <main className={styles.main}>
      ...
      <div className={"d-flex " + styles.bodycontainer}>
        {width > 540 && (
          ...
          >
            <VideoAppointmentSidebar
              ...
              toggle={isCallJOined ? "" : "offcanvas"}
              target={isCallJOined ? "" : "#offcanvasScrolling"}
              controls={isCallJOined ? "" : "offcanvasScrolling"}
            />
        ...
        <div className={styles.audiocall}>
          {width <= 540 ? (
            <button className={styles.openMenu} onClick={() => setShow(!show)}>
              Menu
            </button>
          ) : (
            <span></span>
          ...
          <audio
            className={styles.confrenceContainer}
            id="mediaElement"
          ></audio>
          ...
          <div className={styles.audioContainer}>
            ...
            {showProfile && <img src={userProfile?.picture} alt={"profile"} />}
          </div>
