...
const Index = () => {
  ...
  const getProfRateInfo = async (slotid) => {
    let URL = `${API.walletScheduleDetail}/${id}?clientId=${userProfile?.id}`;
    if (slotid) {
      URL = URL + `&slotId=${slotid}`;
    }
  ...
  useEffect(() => {
    if (id && userProfile?.id) {
      getProfRateInfo(slotId);
    }
  }, [slotId, id, userProfile]);
